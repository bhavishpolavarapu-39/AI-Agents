            ...
            <div className="space-y-2">
              <div>
                ...
                <div className="text-2xl font-bold text-white">
                  {market.current_value?.toLocaleString('en-US', { maximumFractionDigits: 0 }) || 'N/A'}
                </div>
              </div>
              ...
              <div>
                ...
                <div className={`text-lg font-semibold ${market.change_percent >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {market.change_percent >= 0 ? '+' : ''}{market.change_percent?.toFixed(2) || '0'}%
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-green-900/10">
                <div className="text-xs text-slate-500">
                  <span className={`${market.status === 'open' ? 'text-green-400' : 'text-yellow-400'}`}>
                    ● {market.status?.charAt(0).toUpperCase() + market.status?.slice(1) || 'Closed'}
                  </span>
                </div>
        ...
        <div className="mt-6 p-4 bg-slate-800/50 rounded-lg border border-green-900/20">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            ...
            <div>
              ...
              <div className="text-lg font-bold text-green-400">
                {(marketEntries.reduce((sum, [_, m]) => sum + m.change_percent, 0) / marketEntries.length).toFixed(2)}%
              </div>
            </div>
          </div>
        </div>
